public class Area {
    ...
    if (length <= 0 || width <= 0) throw new IllegalArgumentException("Invalid length or widt return length*width;
    }
   }
   public class TestArea { // Regression test for method area
    ...
    if (Area.area(0, 10) != 0) System.err.println("FAIL: 0x10 not 0 but " + Area.area(0,10)); // TEST VECTOR #4: Negative Length Side
    try {
    int i = Area.area(-1, -2);
